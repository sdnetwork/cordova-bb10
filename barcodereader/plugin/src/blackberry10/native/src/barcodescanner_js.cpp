...
BarcodeScannerJS::BarcodeScannerJS(const std::string& id) :
    ...
    fprintf(stderr, "Barcode initialize");
}
...
string BarcodeScannerJS::InvokeMethod(const string& command) {
    ...
    if (strCommand == "startRead") {
        ss << m_pBarcodeScannerController->startRead();
        return ss.str();
    }
    else if (strCommand == "stopRead") {
        ss  << m_pBarcodeScannerController->stopRead();
        return ss.str();
    } else if (strCommand == "pauseRead") {
        m_pBarcodeScannerController->pauseRead();
    } else if (strCommand == "resumeRead") {
        m_pBarcodeScannerController->resumeRead();
    }
